    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex items-center justify-between my-6">
            ...
            <div class="flex gap-2 items-center">
                <input v-model="search" type="text" placeholder="Search categories..." class="rounded border px-3 py-2" />
        ...
        <Card>
            <CardContent>
                <table class="min-w-full divide-y divide-border">
                    ...
                    <tbody>
                        <tr v-for="category in (page.props.categories as Paginated<Category>).data" :key="category.id" class="hover:bg-muted">
                            ...
                            <td class="px-4 py-2 text-sm">{{ new Date(category.updated_at).toLocaleString() }}</td>
                            <td class="px-4 py-2 space-x-2">
                                <Link :href="route('categories.edit', category.id)" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-8 w-8">
                                    <Icon name="edit" class="h-4 w-4" />
                                </Link>
                                <button class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-8 w-8 text-destructive hover:text-destructive" @click.prevent="deleteCategory(category.id)">
                ...
                <div class="flex justify-between mt-4">
                    <button v-if="(page.props.categories as Paginated<Category>).prev_page_url" @click="goToPage((page.props.categories as Paginated<Category>).current_page - 1)" class="btn">Previous</button>
                    <button v-if="(page.props.categories as Paginated<Category>).next_page_url" @click="goToPage((page.props.categories as Paginated<Category>).current_page + 1)" class="btn">Next</button>
                </div>
            </CardContent>
        </Card>
    </AppLayout>
